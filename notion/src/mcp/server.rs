...
impl Server {
    ...
    pub fn add_handler<F, Fut, Req, Res>(&self, method: &str, handler: F)
    where
        ...
        Res: serde::Serialize + Send + 'static,
    {
        ...
        // In a real implementation, you'd use async methods
        tokio::spawn(async move {
            // This is a placeholder - real implementation would store the handler
        });
    }
    ...
    pub async fn connect<T: Transport + Send + 'static>(&self, mut transport: T) -> Result<()> {
        loop {
            match transport.receive().await {
                Ok(message) => {
                    // Process the message
                    if let Ok(method) = message.get("method").and_then(|m| m.as_str()) {
